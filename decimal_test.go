...
func TestNonWholePowRoot(t *testing.T) {
	exp := RequireFromString("2.2795070569547776")
	gotPow := New(3, 0).PowRound(New(75, -2), 16)
	if !exp.Equal(gotPow) {
		t.Fatalf("3^(3/4):\nexpected:%v\n     got:%v", exp, gotPow)
	}

	gotRoot := New(3, 0).RootRound(New(4, 0).Div(New(3, 0)), 16)
	if !exp.Equal(gotRoot) {
		t.Fatalf("4/3 root of 3:\nexpected:%v\n     got:%v", exp, gotRoot)
	}
}
